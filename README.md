...
## 💡 概要 (About) — **堅牢な並列ウェブ抽出とAI処理パイプライン**

**Act Feed Clean Go** は、RSSフィードから記事URLを抽出し、**並列処理**で本文を高速に取得、ノイズ除去を行い、最終的にAIによる**構造化・クリーンアップ**を実行する高生産性 CLI ツールです。
...
AI処理を実行する際には `GEMINI_API_KEY` または `--llm-api-key` フラグによるAPIキーの指定が必須となります。
...
```bash
# 環境変数または -k フラグでAPIキーを指定すると、AI構造化処理が実行される
