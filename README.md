...
| **並行処理** | Goroutines / **`web-text-pipe-go`** | 記事URLのウェブ抽出リクエストを**堅牢なパイプライン制御**の下で並列実行し、処理時間を短縮します。 |
...
| **HTTPクライアント** | **`shouni/go-http-kit` (内包)** | **自動リトライ**と**タイムアウト制御**を持つ高信頼性のネットワーククライアントを提供します。 |
| **ウェブ抽出** | **`shouni/go-web-exact/v2` (内包)** | 記事本文のインテリジェントな特定とノイズ除去を実行します。**`web-text-pipe-go`ライブラリを経由**して使用されます。 |
| **CLIフレームワーク** | **`spf13/cobra`** | ユーザーフレンドリーで強力なコマンドラインインターフェースを構築します。 |

-----

## 🛠️ `web-text-pipe-go`による低レベル処理の抽象化

本ツールは、Webコンテンツ取得に関する低レベルな処理を、新しく導入した`web-text-pipe-go`ライブラリに完全に委譲し、アプリケーションのコアロジックを簡素化しています。

### 1\. Webコンテンツ取得の抽象化

* **高レベルな依存**: アプリケーションは、フィードのパース、Webコンテンツの取得、テキスト抽出を統合的に実行する**`ScraperRunner`**という高レベルな抽象化（インターフェース）に依存します。
* **責務の分離**: 記事取得、フィードパース、抽出の具体的なロジックはすべて`web-text-pipe-go`ライブラリ内部にカプセル化されています。

### 2\. 堅牢な自動リトライとタイムアウト制御

* **ライブラリ内での堅牢化**: 自動リトライやタイムアウト制御を含むネットワーク通信の堅牢性は、`web-text-pipe-go`が内部で利用する[`shouni/go-http-kit`](https://github.com/shouni/go-http-kit)によって担保されます。
* **ユーザー定義可能なタイムアウト**: `--http-timeout` フラグにより、**すべての**Webリクエストの個別タイムアウト時間をユーザーが柔軟に設定可能です。

### 3\. 高度なWebコンテンツ特定ロジック

* **インテリジェントな本文特定**: [`shouni/go-web-exact/v2`](https://github.com/shouni/go-web-exact/v2) のインテリジェントな本文特定ロジックが、**`web-text-pipe-go`を介して**活用されます。
* **ノイズ要素の自動除去**: 広告、ソーシャルシェアボタンなど、記事本文と無関係な要素を自動で除去します。
